...
void GUI::DrawParameterSpace(Ref<Scene> scene, UiState&)
{
	...
	DrawParamCurve("##SquareP", "Surface P", data.paramsP, data.wrappedPU, data.wrappedPV,
		data.componentEnds);
	ImGui::SameLine();
	DrawParamCurve("##SquareQ", "Surface Q", data.paramsQ, data.wrappedQU, data.wrappedQV,
		data.componentEnds);
	...
	trimControls(data.surfaceP, "Surface P");
	if (!data.selfIntersection)
	{
		trimControls(data.surfaceQ, "Surface Q");
	}
