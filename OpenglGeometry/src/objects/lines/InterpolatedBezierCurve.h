...
class InterpolatedBezierCurve : public RenderableOnScene, public IObserver, public ILine
{
private:
	bool displayPolyline = false;
	std::vector<std::weak_ptr<Point>> points;

	inline std::string GetTypeName() const override { return "Interpolated Bezier Curve"; }
...
public:
	InterpolatedBezierCurve(std::vector<std::shared_ptr<Point>> points, SelectedShapes* shapes);
	~InterpolatedBezierCurve()
	{
