...
namespace Geometry
{
	...
	namespace Detail
	{
		inline void StampSegment(std::vector<uint8_t>& wall, int res,
			float u0, float v0, float u1, float v1)
		{
			...
			for (int i = 0; i <= steps; i++)
			{
				...
				wall[y * res + x] = 1;
				if (x + 1 < res) wall[y * res + x + 1] = 1;
				if (y + 1 < res) wall[(y + 1) * res + x] = 1;
			}
		}
	...
	inline TrimMaskData BuildTrimMask(const std::vector<Algebra::Vector4>& params,
	...
	inline bool MaskHasTwoRegions(const TrimMaskData& mask)
	{
		bool sawA = false, sawB = false;
		for (std::size_t i = 0; i < mask.pixels.size(); i += 4)
		{
			(mask.pixels[i] == 0 ? sawA : sawB) = true;
			if (sawA && sawB) return true;
		}
		return false;
	}
}
