...
namespace Geometry
{
	...
	struct IntersectionData
	{
		...
		// One-past-last index of each traced component in the vectors above. Empty or a
		// single entry == the whole polyline is one component (back-compatible).
	...
	// Squared P - Q gap a polished seed must beat to count as a real intersection. A
	// genuine crossing drops to the evaluation floor (~1e-12); a near-miss valley stops
	// far short (the closest approach, squared), so this cleanly rejects false hits.
	constexpr float kSeedGapTolerance = 1e-8f;

	// Distance between the two parameter pairs, folded across any wrapped axis so a
	// seam-straddling pair (v = 0 vs v = 1 on a closed surface) is recognised as the
	// same location. Used to reject the trivial P == Q answer of a self-intersection.
	inline float ParamSeparation(const IParametricSurface& p, const IParametricSurface& q,
	...
	// Minimises the squared gap ||P(uP,vP) - Q(uQ,vQ)||^2 by conjugate gradients
	// (Fletcher-Reeves, restarted every 4 steps) with a backtracking Armijo line
	// search. Returns the parameters of the closest pair; check GapSquared on the
	// result to decide whether it actually landed on an intersection.
	...
	// A self-intersection seed cannot be refined by MinimizeGapCG / PolishSeed: the
	// global minimum of ||P(x) - P(y)||^2 is the trivial x == y diagonal, so both slide
	// the seed straight onto it. This runs the marcher's own 4x4 correction with a zero
	// arc step instead - three rows P(x) - P(y) = 0, one row pinning the seed's position
	// along the frozen crossing tangent n(x) x n(y). The diagonal is not a fixed point
	// of that system: n(x) x n(y) -> 0 there, so the last row degenerates and the solve
	// pushes away from it.
	...
	// Least-norm Gauss-Newton on P - Q = 0 (three equations, four unknowns): each step
	// is the smallest parameter move that removes the current gap, so it stays next to
	// the conjugate-gradient result instead of sliding along the curve or collapsing
	// onto the P == Q diagonal. On a real intersection this converges to the evaluation
	// floor; on a near-miss it stalls at the closest-approach distance - which is how
	...
	// Coarse grid search, each candidate refined with CG then a least-norm Newton
	// polish. For every P sample it keeps the nearest Q sample, then walks the
	// candidates in order of increasing gap - spread over distinct regions and skipping
	// anything a caller-supplied `block` already covers - and returns the first whose
	// polished gap reaches the evaluation floor (`tolerance`). A near-miss polishes to a
	// gap far above that and is passed over. For a self-intersection pass the same
	// surface twice and set `minSeparation` > 0 to reject the trivial P == Q answer.
	// All distinct seeds that survive the polish: one closest-Q candidate per P grid
	// cell, sorted by gap, each refined (self-intersections through PolishSelfSeed, two
	// surfaces through CG + PolishSeed) and kept only if it reaches the evaluation floor
	// and, for a self-intersection, stays off the P == Q diagonal. Near-misses polish
	// short of `tolerance` and drop out. Distinct seeds land the caller on every
	// component - disjoint loops of a two-surface intersection, or every lobe of a
	// self-intersection.
	inline std::vector<SurfaceParam> CollectSeeds(const IParametricSurface& p,
		const IParametricSurface& q, bool self, int grid = 24,
		float tolerance = kSeedGapTolerance)
	{
			...
			for (int b = 0; b <= grid; b++)
			{
				...
				for (int c = 0; c <= grid; c++)
				{
					for (int d = 0; d <= grid; d++)
					{
						...
						if (self && ParamSeparation(p, q, { uP, vP, uQ, vQ }) < minSeparation)
						{
							continue;
						}
		...
		for (const auto& candidate : candidates)
		{
			...
			if (self && ParamSeparation(p, q, s) < minSeparation)
			{
				continue;
			}
	...
	// Gaussian elimination with partial pivoting; solves a x = b in place. Returns
	// false when a is (numerically) singular - near a tangency Newton cannot proceed.
	inline bool SolveLinear4(std::array<std::array<float, 4>, 4> a, std::array<float, 4> b,
		std::array<float, 4>& x)
	{
		x = {};
	...
	// One marching step: correct `x` back onto P = Q while also moving it a signed
	// arc length `arcStep` along `tangent` away from `anchor`. 4x4 Newton, three rows
	// from P - Q = 0 and one from the arc-length plane.
	inline bool NewtonCorrect(const IParametricSurface& p, const IParametricSurface& q,
	...
	// Marches one way from `seed` (sign of `arcStep` picks the direction), Newton-
	// correcting each step and shortening it up to six times when Newton stalls or
	// oversteps. Stops at a tangency, a domain edge, or - after enough samples -
	// when the curve returns to its start (then `closed` is set).
		...
		std::vector<Algebra::Vector4>& paramsQ, bool& closed)
	{
		...
		for (int sample = 0; sample < maxSamples; sample++)
		{
			...
			// n_P x n_Q flips sign whenever either surface normal does; keep the frozen
			// per-step tangent continuous with the last accepted one so the march does
			// not double back on itself (matters at a self-intersection, where the two
			// sheets meet at a shallow angle).
		...
		const IParametricSurface& surfaceQ, bool self, const IntersectionSettings& settings)
	{
		...
		// Collect every distinct seed, trace each, keep the traces that are not already
		// covered. Grazing lobes that share a parameter region defeat any "block the
		// footprint and reseed" scheme, so nothing is blocked - the seeds are found up
		// front and the traces themselves are de-duplicated by proximity.
		std::vector<SurfaceParam> seeds;
		...
		if (settings.useCursor && !self)
		{
			...
			if (GapSquared(surfaceP, surfaceQ, s) <= kSeedGapTolerance)
			{
				seeds.push_back(s);
			}
		}
		...
		{
			seeds = CollectSeeds(surfaceP, surfaceQ, self);
		}
		...
		for (const SurfaceParam& seed : seeds)
		{
			...
			// Discard a trace that wandered - a real component's arc length is only a
			// few times its own extent; a marcher that stalled at a tangency and looped
			...
			// Drop a trace that mostly retraces ground already covered - the swapped
			// (P<->Q) view of a self-crossing, or the same loop from a different seed.
			int matched = 0;
